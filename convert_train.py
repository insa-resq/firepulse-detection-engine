...
# --------- CONFIG -----------
INPUT_DIR = r"C:\Users\mateo\Desktop\satellite\cropped-data1"
MASK_DIRS = [
    r"C:\Users\mateo\Desktop\satellite\cropped-data1",  # same folder as images
    r"C:\Users\mateo\Desktop\satellite\cropped-data1\masks",  # other possible folder
]
OUTPUT_DIR = r"C:\Users\mateo\Desktop\satellite\cropped-data-newer-yolo"
IMAGES_OUT = Path(OUTPUT_DIR) / "images"
LABELS_OUT = Path(OUTPUT_DIR) / "labels"
SPLIT_DIR = Path(OUTPUT_DIR) / "splits"   # will contain train.txt val.txt test.txt (paths)
    ...
    try:
        with rasterio.open(tif) as src:
            ...
            if mask_path is None:
                # no mask found → empty label file (NoFire)
            ...
            with rasterio.open(mask_path) as msrc:
                ...
                if (msrc.width, msrc.height) != (src.width, src.height):
                    # resample using rasterio.warp or simple cv2 resize after normalization
                    # We'll normalize to 0/255 then resize with nearest to preserve classes
                    mask_uint8 = (mask_band > 0).astype(np.uint8) * 255
                    mask_resized = cv2.resize(mask_uint8, (out_w, out_h), interpolation=cv2.INTER_NEAREST)
                else:
                    # mask and source have same native size; if we resized image, resize mask similarly
                    mask_uint8 = (mask_band > 0).astype(np.uint8) * 255
                    if TARGET_SIZE is not None:
                        mask_resized = cv2.resize(mask_uint8, (out_w, out_h), interpolation=cv2.INTER_NEAREST)
                    else:
...
# --- create data.yaml for YOLOv11 training (adjust names and paths as needed)
data_yaml = {
    ...
    'test': str((Path(SPLIT_DIR) / "test.txt").absolute()),
    'nc': 1,
    'names': {0: 'fire'}
}
...
print("YOLO data file:", Path(OUTPUT_DIR) / "data.yaml")
...
# --------- CONFIG -----------
INPUT_DIR = r"C:\Users\mateo\Desktop\satellite\cropped-data-news"
MASK_DIRS = [
    r"C:\Users\mateo\Desktop\satellite\cropped-data-news",  # same folder as images
    r"C:\Users\mateo\Desktop\satellite\cropped-data-news\masks",  # other possible folder
]
OUTPUT_DIR = r"C:\Users\mateo\Desktop\satellite\cropped-data-newer-yolo"
IMAGES_OUT = Path(OUTPUT_DIR) / "images"
LABELS_OUT = Path(OUTPUT_DIR) / "labels"
